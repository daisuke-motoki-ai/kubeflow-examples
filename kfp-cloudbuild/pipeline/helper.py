...
def update_component_spec(repo_url, image_tag):
    ...
    for spec_path in pathlib.Path("components").glob("*/component.yaml"):
        ...
        print(
            "Component {} specs updated. Image:{}".format(image_name, full_image_name)
        )
...
def deploy_pipeline(kfp_package_path, version, experiment_name, namespace, host, run):
    ...
    if run:
        ...
        client.run_pipeline(
            experiment.id, job_name=run_id, pipeline_id=pipeline_id, params=settings
        )
...
def main(operation, **args):
    ...
    # Deploy Pipeline
        ...
        raise ValueError(
            "Invalid operation name: {}. Valid operations: update-specs | deploy-pipeline".format(
                operation
            )
        )
